...
public:
    ...
    void conduct_experiment(Rat &roger) {
        ...
        while (!roger.completed_route() && i <= 1000) {
            ...
            /*while ((roger.cur_cell_int == 18 || roger.cur_cell_int == 20) && (!a || !g)) {
                cur_generated_route.pop_back();
                map_from_name_to_operator[chosen_operator]->apply(*this, roger);
            }*/
    ...
    void print_all_generated_routes(ostream &o) {
        ...
        for (size_t i = 0; i < vector_of_generated_routes.size(); ++i) {
            ...
            for (int j = 0; j < vector_of_generated_routes[vector_of_generated_routes.size() - i - 1].size(); ++j) {
                char k = int_to_char[vector_of_generated_routes[vector_of_generated_routes.size() - i - 1][j]];
                if (k >= "А" && k <= "Я") {
                    o << k;
                }
            }
...
/*void Operator_Corner::apply(Maze &maze, Rat &rat) {
    //Логика оператора в том, что обезьяна проходит контрольный пункт и почти всегда сворачивает
}*/
...
void Operator_Compression::apply(Maze &maze, Rat &rat) {
    ...
    int len = 7; // опять же случайная константа
    if (path.size() < len) {
        //len = path.size();
        return;
    }
    ...
    for (int i = 1; i < path_to_compress.size(); ++i) {
        if (path_to_compress[i] == 1 && !S && path_to_compress[prev] != path_to_compress[i]) {
            S = true;
            vector<int> shortest_distance_to_S = maze.direct(path_to_compress[prev], path_to_compress[i]);
            for (int j = 1; j < shortest_distance_to_S.size(); ++j) {
                shortest_distance.push_back(shortest_distance_to_S[j]);
            }
            prev = i;
        }
        else if (path_to_compress[i] == 4 && !F && path_to_compress[prev] != path_to_compress[i]) {
            F = true;
            vector<int> shortest_distance_to_F = maze.direct(path_to_compress[prev], path_to_compress[i]);
            for (int j = 1; j < shortest_distance_to_F.size(); ++j) {
                shortest_distance.push_back(shortest_distance_to_F[j]);
            }
            prev = i;
        }
    }
    if (path_to_compress[prev] != path_to_compress[path_to_compress.size() - 1]) {
        vector<int> last_short = maze.direct(path_to_compress[prev], path_to_compress[path_to_compress.size() - 1]); // cтроим кратчайший путь
    ...
    //vector<int> shortest_distance = maze.direct(path[path.size() - len], path[path.size() - 1]); // cтроим кратчайший путь
...
// напишем вспомогательную функцию для вычисления вероятностей применения операторов

//Реализация метрики Левенштейна

int LevenshteinDistance(std::string st_1, std::string st_2) {
    if (st_1.size() > st_2.size()) {
        return LevenshteinDistance(st_2, st_1);
    }
...
int lvn(string &now, vector<string> &all_routes) { //вычисление метрики Л. для каждой пары (сгенерированная последовательность, реальный матршут)
    ...
    for (int i = 1; i < all_routes.size(); ++i) {
        int ans_now = LevenshteinDistance(now, all_routes[i]);
        if (ans_now < minim) {
            minim = ans_now;
        }
    }
...
vector<double> optimal(Maze &maze, Rat &roger, vector<string> &all_routes, ofstream &out, std::vector<std::vector<double>> grid, int &save) {
        vector<double> optimal = {};
        vector<int> l = {};
        for (int i = 0; i < grid.size(); i++) {
            /*maze.set_probabilities_for_operators({0.0, grid[0][0], grid[0][1], grid[0][2], grid[0][3]});
            string m = maze.conduct_series_of_experiments_memorizing_routes(1, roger);
            int save = LevenshteinDistance(m, последовательность 2);*/
            maze.set_probabilities_for_operators({0.0, grid[i][0], grid[i][1], grid[i][2], grid[i][3]});
            ...
            for (int j = 0; j < maze.vector_of_generated_routes[0].size(); j++) {
                if (maze.vector_of_generated_routes[0][j] >= 1 && maze.vector_of_generated_routes[0][j] <= 33) {
                    k += int_to_eng[maze.vector_of_generated_routes[0][j]];
                }
            ...
            int save_min = lvn(k, all_routes);
            //out << "Левенштейн = " << save_min << '\n';
            //out << "Вероятности " << grid[i][0] << " " << grid[i][1] << " " << grid[i][2] << " " << grid[i][3] << '\n';
            if (save_min < save) {
                save = save_min;
                optimal = grid[i]; //нужно сохранить оптимальные вероятности для всех операторов
            }
        }
        return optimal;
...
int main() {
    ...
    std::ifstream in_for_u("untitled.txt");
    ...
    //maze.set_probabilities_for_operators({0.0, 90 / delimiter, 10 / delimiter, 0 / delimiter, 0 / delimiter});
    //maze.conduct_series_of_experiments_memorizing_routes(3, roger);
    int save = 100000; 
    std::vector<std::vector<double>> grid = gen_grid({ 0.1, 0.1, 0.1, 0.1 }, { 0.9, 0.9, 0.9, 0.9 });
    
    vector<string> all_routes;
    string line;
    while (getline(in_for_u, line)) { //сохранение всех реальных последних маршрутов в вектор
        all_routes.push_back(line);
    }
    
    

    std::vector<double> opt = optimal(maze, roger, all_routes, out, grid, save);// h
    out << "\n";
    out << "Наиболее выгодный набор вероятностей: " << "Рандом = " << opt[0] << " Инверсия = " << opt[1] << " Симметрия = " << opt[2] << " Сжатие = " << opt[3] << "\n";
    out << "Метрика Левенштейна: " << save << "\n";

    maze.set_probabilities_for_operators({0.0, opt[0], opt[1], opt[2], opt[3]});
    maze.conduct_series_of_experiments_memorizing_routes(3, roger);
    ...
    if (maze.vector_of_generated_routes.size() != 0) {
        for (int i = 0; i < maze.vector_of_generated_routes.size(); ++i) {
            ...
            int save_min = lvn(k, all_routes);
            out << "\nНабор вероятностей: " << "Рандом = " << opt[0] << " Инверсия = " << opt[1] << " Симметрия = " << opt[2] << " Сжатие = " << opt[3] << "\n";
            out << "Значение метрики: " << save_min;
